...
- **The other arm of that rule: to keep a peer genuinely optional, load it with `try { require('<string literal>') } catch` at the boundary — never a static import, and never `lazyRequire()`.** Metro collects a literal `require` inside a `try` as an OPTIONAL dependency: it resolves the real module when installed and writes `null` into the dependency map when it is not, so an absent peer fails at evaluation instead of aborting the bundle. `lazyRequire()` does NOT qualify — it passes the specifier as a variable, and Metro rewrites `require(variable)` into a thrower ("Dynamic require defined at line N; not supported by Metro"), so from a consumer's `node_modules` it resolves nothing whether or not the package is installed (same fact as the "never lazy `require('nativewind')`" note under Typography). Choose this arm only when the subpath still does something useful without the peer — `connection-status` goes inert, costing the outage toast and nothing else — and a required peer when it does not (`tab-bar`). Either way the declared range must be real: an empty string constrains nothing and no package manager can act on it. Reference implementation `src/connection-status/netinfo.ts`; gate `src/__tests__/optional-peer-imports.test.ts`, which also pins the documented allowlist of static imports that are safe because the package cannot actually be absent (`expo-font` ships inside `expo`, `react-dom` comes with react-native-web, `expo-router` is what scopes `BloomProvider` to expo-router apps).
