...
- **`scripts/generate-platform-exports.mjs` OWNS `package.json#exports`, `#typesVersions` AND every web barrel** — hand-editing any of them is silently reverted by the next `prebuild`. `typesVersions` exists for `moduleResolution: "node"`, which ignores `exports` outright; before it, NO Bloom subpath typechecked under that setting (measured, TS 5.9). It is never consulted under `node16`/`bundler`, so it can only ever add.
- **A `*` in an `exports` key must carry every character that makes the match unambiguous.** `./icons/Ri*` not `./icons/*`: the bare form also matches `index`, and the conditions then DISAGREE — the built `icons/remix/index.js` exists while `src/icons/remix/` has an `index.ts` and no `index.tsx`, so `@oxy.so/bloom/icons/index` resolves everywhere except Metro, the one consumer that reads `src/`. `existsSync` cannot be asked about a path holding a `*`, so nothing else here sees a pattern target: `verify-package.mjs` expands each one against the real tarball and asserts every condition yields the SAME names.
...
Five small contexts a container publishes and a component opts into. The page has the model; these are the traps.

- **A control joins a `Field` through `useFieldMembership()`, never by hand** — four decisions with four directions, and each is silently wrong reversed: the NAME defers to the field (`??`), `disabled`/`invalid` COMBINE with it (`||`, a constraint), the ID prefers the caller's, `aria-describedby` is a LIST and is JOINED. **`labelPlacement` decides which label wins**: a label drawn ABOVE the control (`TextField`, `Textarea`) yields to the field's; one drawn BESIDE it (a checkbox caption) keeps its own, or the announced name contradicts the screen. **A composite must hand the state to the halves that read nothing** — `PhoneInput`'s country picker stayed operable inside a disabled field because only the input read the context. Gates: `FieldMembership.test.tsx` (15 families, mutation-verified: flipping one `||` to `??` reddens 18) + `FieldAssociation.test.tsx`.
- **`density` is `'medium' | 'small'` and nothing else** — a family whose sizes are a three-rung scale of its own is NOT a subject; a container setting `density` could not say which rung it meant. `TextField`/`Textarea`/`TimeField` read it, `InputGroup`'s geometry outranks it (its members must be one height), `Button` deliberately reads nothing.
- **A modal or sheet is its OWN SCREEN — wrap it in `layout/ScreenScope`** or its chrome claims the top edge of the page underneath (padding that appears and disappears with the modal) and follows a scroller nobody is touching. `DialogBody` and `BottomSheetBase` mount it, so all three dialog placements and every sheet get it in one place. Gate: `ScreenScope.test.tsx`, both directions.
- **`docs/adoption-matrix.mdx` is GENERATED** (`bun run generate:adoption-matrix`; derivation in `src/__tests__/support/adoption-matrix.ts`, so the gate and the generator cannot disagree) — hand-editing it is reverted by the next run. Applicability is derived from what a family RENDERS, and every applicable family that does not read a contract is CLASSIFIED with a reason, as an EQUALITY: a new one fails the gate as `UNCLASSIFIED`, a stale one fails it too. **Do not resolve conformance transitively** — an earlier version did, and since nearly every family reaches `text-field` eventually, 17 families rendering raw `TextInput`s came out green.

## Getting a COMPLETE test pass: shard it

**`bun run test` on this repo can die mid-run**, taking a worker with it: `A jest worker process was terminated by another process: signal=SIGSEGV|SIGTRAP|SIGABRT`, sometimes preceded by `# Fatal error in , line 0`. It is Node's `vm`/contextify, not this code — the victim suite is arbitrary (`glass-colors` most often, but four different colour suites at `--maxWorkers=2`, and six on another tree), it reproduces on `main` as well as on a branch. **It has WORSENED: as of 2026-09-07 on Node 24.20.0 the victim no longer passes on its own** — `theme/__tests__/glass-colors.test.ts` and `theme/__tests__/color-preset-registry.test.ts` die alone AND under `--runInBand`, with `FATAL ERROR: Context::GetNumberOfEmbedderDataFields Not a native context`; `theme/__tests__/theme-colors-parity.test.ts` joined them by 2026-09-17, verified by running it on an unmodified `origin/main` checkout — **check a suspect against main before believing your branch broke it.** All three still pass inside a full unsharded run, which is how they were last seen green. Node 24.14.1 dies the same way; Node 22 is NO control — it cannot load `jest.config.ts` without `ts-node`. So a complete pass here is currently 191 of 193 suites, and those two are the shortfall to name rather than to hide.
...
- **A SURFACE is never a ramp stop — ask `styles/surface-levels.ts`.** A control cannot know its parent, so `surfaceFillOn(theme, parent)` / `hairlineOn(theme, parent)` / `surfaceTextOn(theme, parent)` take the fill they land on, and a container that paints one publishes it with `SurfaceLevelProvider` (`FloatingPanel`, `QueuePanel` do). Picking `n[800]` instead is how the dark text field landed byte-for-byte on the menu surface (1.000:1, no shell) and the dark tab rail on the queue panel (ΔE 0) — both VALID markup, both invisible, so only a measurement catches them. Gates: `surface-levels.test.ts`, `surface-level-adoption.test.ts`.
- **A quiet text colour is `quietText(surface, text, floor)`, never a ramp stop** — `n[400]` clears AA on the page and measures 2.42:1 on a chart card. `quietTextOver([...])` when the label lands on more than one fill. `theme.colors.textTertiary` IS `textSecondary` (`--muted-foreground`): one role, one colour, since a fixed third grey is measured against one surface and used on all of them (`docs/design-tokens.mdx`).
...
- **Never hand-roll a determinate bar or ring** — `stat-bar`'s `Meter`/`MeterRing` own the geometry, the ACCENT fill, the `neutral-200`/`neutral-700` track and the flat `aria-value*` props. Seventeen copies disagreed: three filled with `colors.text` (ink, not a measurement), rails ran 200/700, 200/800, 300/700 and 100/800. A segment of a bigger progressbar is `decorative` (no role), never a second one. **A CHART is not a meter** — if a bar's colour says WHICH datum it is, colour it from `chart-cards/palette.ts` (`chartHueTone`/`resolveMonoTone`); `neutralSeries` is `neutral-800` in dark and vanishes on a rail. Gates: `Meter.test.tsx` + the `Meter` subjects in `aria-state-web.test.tsx`.
...
- **ONE focus-ring gap and ONE disabled opacity.** The gap is `var(--bloom-ring-offset)` from `useRingOffsetStyle()`; as the literal `#FFFFFF` it drew a 2px white halo in DARK across ten components. Disabled RECOLOURS by token where a pair exists (`Button`); where it fades it fades by `tokens.DISABLED_OPACITY` (0.5) — 0.4 and 0.6 were in the tree with nothing distinguishing them. Build a raw-DOM or `dataSet` control's sheet with `interactiveWebCss()` (`reset: 'none'` for react-native-web — the `<button>` reset's `inline-flex` changes an RNW `View`'s layout); hand-writing one is how all three drifted. Gate: `interactive-recipe.test.ts`.
