...
- **The LABEL follows the alpha and flips with it** — at 0.25 the pane took its luminance from the page (`colors.text` right, on-fill failed 263/1260); at 0.85 the pane IS the fill (on-colour right, `colors.text` fails 1015/1260). Re-measure both on every alpha change; the gate walks them side by side.
- **The gate pins 0.85's AA cost as an EXACT count plus preset/mode set** (`docs/button.mdx`), so a one-hundredth alpha change goes red either direction.
...
- **Prove translucency DIRECTLY.** "Still fails AA over black/white" stops discriminating near opaque. Measure whether the PAINTED pane moves with the backdrop: 0 at opacity, monotonic in alpha (0.85→37.5, 0.90→25.0, 0.95→12.5, 1.00→0.0 on web). Floors must be LITERALS, not derived from the alpha, or both sides move together and measure nothing.
- **Glass replaces a FILL, never adds one** — a blur behind a transparent control shows nothing, and the hairline reads as the pane's edge only where there's a tint. **`inverse` stays opaque** as the backdrop-independent CTA (`docs/button.mdx`); less load-bearing at 0.85 than at 0.25 — re-measure rather than repeat either number.
