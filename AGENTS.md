...
- **Name the backdrops a translucent surface must survive BEFORE calibrating opacity.** "Clears AA over black AND white" is arithmetically fine and forces 0.76 opacity — a near-opaque card with a blur behind it. It shipped once: brand at 3% of the pixel against the reference's 25%, reading as near-white with a coloured rim. A `Button` sits on Bloom's five neutral surfaces, an ENUMERABLE range, over which 25% clears AA everywhere (1080 rows, worst 5.39 web / 5.82 native).
- **Gate BOTH halves or the pass is satisfiable by making the pane opaque** — AA on Bloom surfaces AND a large share still FAILING over black/white. Mutation-verified.
- **The hue was never the problem.** At 25% over a light page Bloom's primaries span L 78–87.5% vs the reference's 90.3%, and half the presets are MORE chromatic. `mono` is grey glass, correct for a greyscale preset.
- **Glass replaces a FILL, never adds one** — a blur behind a transparent control shows nothing, and the hairline is "the edge of the pane" only where there is a tint. **`inverse` stays opaque BY MEASUREMENT:** over unknown content half the matrix falls below AA (worst 1.02), as does the reference itself (4.05 over a mid photo).
- **`expo-blur` cannot give a radius without a tint** (one `intensity` drives both); `backdrop-filter` is a pure blur. Native carries one neutral layer web does not — 5.39 vs 5.82. Do NOT reproduce expo-blur's tint on web to "match".
- **A glass variant must not set `overflow: 'hidden'`** — `GlassSurface` self-clips, and `clipsToBounds` on iOS clips the drop shadow away.
- **Read the PAINTED PIXEL** — a translucent surface reports a plausible `background-color` for a slot it is not using, so a computed-style diff cannot tell tinted glass from a washout. Screenshot, reload the capture into the page, sample on a canvas.
