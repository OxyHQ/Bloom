...
- **`restorePending` on the binding lets a caller hide content until the offset lands.** It falls on whichever comes first — stuck, frame budget exhausted, or user takeover — because waiting past any of those waits for something that has stopped trying. It is SEEDED from the store on the first render, not defaulted to `false`: the focus effect is a passive effect and runs after a paint, so a `false` start gives exactly the one wrong-position frame the flag exists to prevent. Consumers must keep the hidden content LAID OUT (`opacity: 0`); `display: none` collapses the document and every write clamps to 0. Web reports it only for the multi-frame loop (a reset writes synchronously, so nothing is pending); native reports it for any deferred write, reset included.
