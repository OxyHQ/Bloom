...
> Org-wide engineering standards (TypeScript, React, naming, error handling, security, testing, git, bun) live at <https://github.com/OxyHQ/engineering/blob/main/AGENTS.md> and are not repeated here. This file holds ONLY Bloom-specific content.
...
Platform-export generation script: `scripts/generate-platform-exports.mjs`. Every subpath with platform-specific behavior ships `.native.ts` + `.web.ts` + a clean default `.ts` with no Metro-only / NW5-only imports. Augmenting `ScrollView`/`FlatList` with `className` in this published RN package requires a heritage-free `declare module 'react-native'` block loaded via `/// <reference path>`. Consumer-facing rules are in parent `~/Oxy/AGENTS.md`.
...
- **Self-inject any CSS a `.web.tsx` fork needs — never make consumers copy it into their global stylesheet.** If a fork needs `@keyframes`, pseudo-classes, or anything a plain inline `style` object can't express, inject it itself: a hook calling `adoptStyleSheet(STYLE_ID, CSS)` from an effect (`Dialog.web.tsx`'s `useDialogCss()`). A raw-DOM CONTROL does not write that CSS by hand at all — `Button.web.tsx`, `Fab.web.tsx` and `FrostedIconButton.web.tsx` build it with `styles/interactive-web-css.ts`'s `interactiveWebCss()` and adopt it with the shared `useInteractiveWebCss()`, which is where the `:focus-visible` and disabled-filter rules are decided once. "Copy this exported CSS string into your app's global CSS" is not an acceptable pattern — an unresolvable `animation-name` fails silently in the browser (no console error, the property just never animates), so a consumer can ship for a long time with dead animations before anyone notices. Any exported raw CSS string (e.g. `BLOOM_DIALOG_CSS`) is for reference/testing only, not something a consumer is expected to wire up.
- **The injection goes through `styles/adopt-style-sheet.ts` — never `document.createElement('style')` with text content.** A `<style>` element whose text content carries the CSS is what a page under `style-src 'self'` blocks, and it fails the worst way available: the rules are dropped, the component renders with none of its CSS, nothing throws, and the fork's own `getElementById(STYLE_ID)` guard finds the element and reports success. A CONSTRUCTED sheet (`new CSSStyleSheet()` + `replaceSync()` + `document.adoptedStyleSheets`) is outside CSP's inline hooks entirely, so the same CSS applies under a strict policy with no nonce and no consumer wiring. Astro's `astro://` WebUI pages ship that CSP; anything else behind one fails identically. `adoptStyleSheet(id, css)` is idempotent and also handles the theme-dependent case (new CSS under the same id replaces in place — `BottomSheetBase`'s scrollbar colours); `dropStyleSheet(id)` detaches. Element-level CSSOM writes (`root.style.setProperty`, which is how `applyColorPresetVars` writes the theme, and React's `style` prop) are NOT affected — CSP governs the parse of a style element's contents and of a `style` ATTRIBUTE, not CSSOM mutation. Consequence to know: on a browser with the constructed API there is no longer a `<style id="bloom-fonts">` element to look for — check the adopted sheets instead.
- **Jest cannot see this on its own.** jsdom 26 ships the `CSSStyleSheet` constructor but neither `replaceSync` nor `document.adoptedStyleSheets`, so every suite takes the `<style>` fallback and a mutation deleting the whole adoption branch stays green. `src/__tests__/support/constructed-style-sheets.ts` installs a spec-shaped stand-in; `adopt-style-sheet.test.ts` asserts *no* style element is created on that path (mutation-verified: disabling the branch fails 8 cases), and `apply-font-faces.test.ts` covers the one injection `BloomThemeProvider` performs for every consumer by default.
...
Compound components are flat-prefixed exports (e.g. `Tabs`, `TabsTrigger`, `TabsContent`; `Menu`, `MenuItem`, `MenuTrigger`). The collection families `Icons`, `Typography`, `Skeleton`, `Grid`, `Code`, `Fonts` stay namespaces.
...
## Accessibility State on Web (CRITICAL — silent, and every control had it)

**`accessibilityState` reaches NATIVE ONLY. On web it is dropped entirely.** react-native-web's `createDOMProps` reads the `aria-*` props; the string `accessibilityState` appears nowhere in that module. So a control setting only `accessibilityState={{checked}}` renders a `role="checkbox"` carrying no state at all — drawn correctly, announced as nothing. Found in `Checkbox` by an Astro agent reading rendered DOM, then in twelve more controls by audit: `Accordion`, `Collapsible`, `Chip`, `CompositionBar`, `Item`, `SegmentedControl`, both `Select` forks, `TabBarBase`, `FrostedIconButton`, `Button` (busy), `Slider`, `InputGroup`, `SettingsList`.

- **The spelling that works on BOTH platforms is `aria-*`.** React Native's `Pressable`/`View` fold `aria-busy|checked|disabled|expanded|selected` back into `accessibilityState`, and `aria-value{min,max,now,text}` into `accessibilityValue`, with the `aria-*` value winning — so setting the aria prop alone is correct everywhere. Prefer one prop over two.
- **`aria-pressed` is the exception: React Native has no such concept** (`AccessibilityState` is `disabled|selected|checked|busy|expanded`, and `aria-pressed` appears nowhere in RN's source). A toggle with `role="button"` must therefore set BOTH — `accessibilityState={{selected}}` for native, `aria-pressed` for web. That is a real platform split, not redundancy.
- **Which aria state is correct depends on the ROLE, so there is no blanket substitution.** ARIA allows `aria-selected` on `tab`/`option`, `aria-checked` on `radio`/`checkbox`/`switch`, `aria-pressed` on `button`. `SegmentedControl` renders `role="tab"` OR `role="radio"` from the same component and needs both branches; a single spelling is invalid for one of the two modes.
- **`aria-disabled` inverts between `Pressable` and `View`.** react-native-web's `Pressable` appends its own `aria-disabled` from the `disabled` prop AFTER spreading the caller's props, so a caller-supplied one is silently overwritten and the `disabled` PROP is the only route. A plain `View` has no `disabled` prop, so `aria-disabled` is the only route. Getting this backwards fails silently in both directions.
- **`accessibilityValue={{min,max,now}}` is dropped too** — react-native-web has no handling for the object form, only the flat `aria-value*` props. `Slider` shipped a `role="slider"` that never announced its value.
- **A prop-level test cannot catch any of this, and that is why it shipped.** Every one of these controls had a passing test asserting `accessibilityState` was set. Assert the rendered ATTRIBUTE instead. Gate: `src/__tests__/aria-state-web.test.tsx`, which runs against REAL react-native-web (`jest.mock('react-native', …requireActual('react-native-web'))`) and is mutation-verified — each fix removed in turn makes it red and names the component.
...
Each family ships as a subpath (`@oxyhq/bloom/dialog`, `@oxyhq/bloom/bottom-sheet`, `@oxyhq/bloom/image-resolver`, `@oxyhq/bloom/skeleton`, etc.), plus the root `index.ts` barrel. Platform exports are generated by `scripts/generate-platform-exports.mjs`.
...
Only TWO overlay surfaces exist. `CenteredDialog` and `ResponsiveSheet` were REMOVED (no shims — consumer guidance in parent).
...
- **That includes the ROLE vocabulary, which it did not always cover.** Both scope builders once emitted `--color-<canonical>` only, so `--color-fill`, `--color-text`, `--color-border-secondary` and the rest of the `bg-fill` / `text-text-*` names — the vocabulary this file tells consumers to prefer — kept resolving against the document root. The symptom is mode-dependent and silent: a forced-dark hero in a LIGHT page painted `text-text` with the root's dark-on-light foreground, i.e. near-black text on a dark photograph, while `--foreground` inside the scope was correct. `withScopeAliases` (`theme/color-scope/seed-scope.ts`) is now the one place both builders go through, and the gate reads the alias list out of `bloomThemeCss()` so an alias added there is covered the day it lands. Anything scope-shaped must go through that helper.
...
- Built-in color presets are the keys of `APP_COLOR_PRESETS` (`src/theme/color-presets.ts`), which is the list, not this file: teal, blue, green, yellow, red, purple, pink, sky, orange, mint, oxy, faircoin, pumpkin, gray, brown, peach, rose, mono (`mono` is the colourless one — a seed at or below chroma 6 takes the greyscale scheme, so a hand-picked grey lands there too; only the status family stays semantic)
...
- **`peerDependencies` + `peerDependenciesMeta` in `package.json` are the list. Never restate the ranges here.** A prose copy of them goes stale silently, and it goes stale INCOMPLETE, which is worse: the copy this file used to carry named six peers and omitted the Apple-only ones the two paragraphs below argue are load-bearing, so it read as permission to drop a peer the package genuinely needs.
- **`@gorhom/bottom-sheet` is NOT a peer and not a dependency of any kind.** The bottom sheet is Bloom's own; the name survives only in `bottom-sheet/BottomSheetBase.tsx` and `dialog/DialogBottomSheet.tsx` comments describing the pan-coordination behaviour they copy. Nothing imports it.
- **A statically-imported peer is never `optional`, whatever its platform support.** `expo-glass-effect` (floored at the release that added `isGlassEffectAPIAvailable`, which `tab-bar/surface.native` imports by name) and `expo-symbols` have Apple-only NATIVE modules (`"platforms": ["apple"]`), which is an argument about what links, not about what resolves: `tab-bar/surface.native` / `tab-bar/glyph.native` import them statically, so omitting them does not degrade — Metro fails to resolve and the native build dies, on Android as much as on iOS. Marking them optional would only silence the warning that says so. Same reasoning keeps `expo-blur` and `expo-image` required, with the platform argument not even available: both are apple+android and both are imported from platform-NEUTRAL files (`progressive-blur/index.tsx`, `frosted-icon-button/FrostedIconButton.tsx`, `zoomable-image-gallery/`).
