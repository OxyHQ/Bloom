...
- **Export conditions do NOT apply to relative specifiers** — EVERY web-only file, barrel or not, must NAME the `.web` sibling of any forked target, or Metro-web is right while Vite/webpack/SSR silently get native. Measured: `AvatarGroup.web.tsx` wrote `'../portal'` where `Dialog.web.tsx` wrote `'../portal/index.web'`, so off Metro the hover card rendered NOTHING (the native `Portal` needs a `PortalOutlet`, which a web app never mounts — both are web no-ops). **Reachability is the wrong half of the property and stays green**: the fork WAS named, by someone else. Storybook's Vite adds `.web.tsx` to `resolve.extensions`, so the browser agreed with the broken build. A NEUTRAL module keeps the bare name — naming `.web` breaks native, and bob leaves a specifier bare exactly when the target has platform variants, so the choice reaches the consumer's bundler intact. Gate: `web-fork-reachability.test.ts`, both properties.
...
**`accessibilityState` reaches NATIVE ONLY — react-native-web drops it entirely** and reads only `aria-*`, which RN folds back (`aria-busy|checked|disabled|expanded|selected`). A control setting just `accessibilityState={{checked}}` renders a role carrying no state. **`aria-pressed` is the exception** — RN has no such concept, so a toggle with `role="button"` must set BOTH, and a component rendering two roles needs both branches. **`accessibilityValue={{min,max,now}}` is dropped too** — only the flat `aria-value*` props work. Full mechanism, including the `aria-disabled` inversion between `Pressable` and `View`: `docs/switch.mdx`, `docs/slider.mdx`.
...
- **`expo-blur` cannot give a radius without a tint** (one `intensity` drives both); `backdrop-filter` is a pure blur. Assert the gap small AND non-zero, or a native stack that silently dropped the blur passes. **Android drops it today** — the `blurMethod`/`blurTarget` cause, why only a TEXTURED backdrop discriminates, and the per-channel gap are in `docs/button.mdx` and `glass-colors.ts`.
