...
`MenubarTrigger` draws its own chrome — shadcn's `rounded-md px-2 py-1.5 sm:py-1` with a `text-sm font-medium` label, washed with the accent colour while its menu is open. Pass a string and it is set for you; pass nodes and they go inside that button; pass `asChild` and your own element replaces the whole thing.
...
Sub-menus render as an inline disclosure on both platforms — see [DropdownMenu](/docs/bloom/main/dropdown-menu).
