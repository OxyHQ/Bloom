...
actions={
  <>
    ...
    <Button appearance="solid" tone="accent" size="md">Save</Button>
  </>
}
```
...
| `scrollY` | — | A Reanimated `SharedValue<number>`. Omitted, the header reads its `Screen` offset, then `ScrollOffsetProvider`, then `window.scrollY` on web. |
...
1. the `scrollY` prop,
2. the enclosing `Screen` offset,
3. the nearest `ScrollOffsetProvider` (`@oxy.so/bloom/layout`),
4. `window.scrollY` — **web only**.
...
**Not verified on a device.** Every number above is Chrome on Linux. iOS and Android have not been run for this change: the material is `expo-blur` on both and is unchanged from the `GlassSurface` the library already shipped, but the geometry, the safe-area padding, the top-edge claim and the scroll linkage have only been exercised in jest and in a browser. Android additionally paints **no blur at all** here, by the design described above, so what a device would show is the tint, the hairline, the sheen and the rim.
...
**Icon actions want `ButtonGroupItem`.** A `Button appearance="outline" tone="neutral"` in a floating header's actions paints its own opaque fill, which reads as a card floating over the content. It still works; it is just not the shape the islands are for.
...
The default scrim and bar background inherit the containing surface’s published
fill (`SurfaceLevelProvider` / `ContentPanel`), falling back to the page
background outside a surface. An explicit `scrimColor` still overrides the fade.
The header’s outer box should span that surface; its own title/action padding
provides the interior spacing.


### Reveal after an in-content heading

`titleReveal="onScroll" titleRevealOffset={headingHeight}` holds both title and subtitle until that scroll distance, then fades them in across `scrollThreshold` (default20px). The scrim remains independent. Hidden header titles leave the accessibility tree until they start appearing; back and action islands remain available.

Measure the in-content heading block with its `onLayout` and pass its height as `titleRevealOffset` when it is the first block below the header. For later headings include the preceding content height. `Screen` and `ScreenScrollView` supply the shared offset; no consumer scroll listener is needed. See **Blocks / Page Header / Reveal After Content Heading** for the complete live composition. The default offset0 preserves existing reveal timing.

### Banner and docked tabs

Use `placement="overlap"` inside `HeaderDockProvider` when a banner should flow
behind the header. Unlike `overlay`, it retains document stickiness on web.
Pair it with `StickySection` from `@oxy.so/bloom/layout`: the section docks below
the measured header and both converge to the inherited opaque surface fill.
`titleReveal="onDock"` reveals the title and subtitle with this shared docking
progress, without an application scroll threshold. At rest the hidden title is
also excluded from accessibility and hit testing. It requires a docking provider
and section; without a target the title remains hidden.

The provider accepts the existing `scrollY` shared value or inherits `Screen`'s.
Native lists retain sticky ownership and consume `useHeaderDockInset()` as their
supported sticky-header offset; see [layout](./layout.mdx). Storybook's
**Profile Docking** demonstrates the document-scroll composition.

For a native list behind the header, use `placement="overlay"` and let the list
consume `useHeaderDockInset()` for sticky clearance. This absolute header has
both a local stacking index and native elevation: Android scroll surfaces can
otherwise paint over its controls. `placement="overlap"` is the document-scroll
composition for web. Keep the header and list in the same unclipped parent;
no descendant elevation can escape an ancestor's clipping bounds.
