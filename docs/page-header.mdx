...
| the island is translucent | the same back capsule over the page and over a photograph | moved **34** of 255 |
| the edge ends in nothing | the vertical ramp, top to page colour | travelled **187** of 255 |
| no horizontal cut | the largest single-row step down that ramp | **4** of 255 |
| control | the same row sampler on `presentation="bar"` | **0** runs — a strip is one colour |

Two of those readings deserve their real explanation rather than a round number. In **light** mode the sampler reports two runs, not three: the two action islands are 8px apart and their drop shadows meet in the gap, so at a 1-level tolerance the gap is not backdrop. The gap is visible; it is not *measurably* backdrop. In **dark** mode, where the shadow is weaker against a dark page, all three separate cleanly. And the island fill in light mode is the card fill over a near-white page — the same relation a `Card` has to the page, read by its hairline and shadow rather than by its fill, which is deliberate and is why it is consistent with the rest of Bloom.
