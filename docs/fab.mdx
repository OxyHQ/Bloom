...
bar. An extended FAB can also react when that bar minimizes:
...
`collapse` retains the icon-only action. `hide` removes the whole action. The
default is `none`, which preserves the full FAB.
