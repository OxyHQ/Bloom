...
`@oxy.so/bloom/zoomable-image-gallery` and
`@oxy.so/bloom/zoomable-media-gallery` are two specifiers for the SAME module.
...
import type { GalleryMedia } from '@oxy.so/bloom/zoomable-media-gallery';
