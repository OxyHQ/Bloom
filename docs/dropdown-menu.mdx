...
```tsx
import { RiCheckLine as CheckIcon } from '@oxyhq/bloom/icons';
...
**A web consumer must have the Tailwind/NativeWind pipeline wired and `@source`-scanning `@oxyhq/bloom/lib`.** That was always required; without it the rows keep their colours (inline, below) but lose every piece of geometry.
