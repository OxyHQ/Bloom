...
```tsx
import { Check_Stroke2_Corner0_Rounded as CheckIcon } from '@oxy.so/bloom/icons';
...
**A web consumer must have the Tailwind/NativeWind pipeline wired and `@source`-scanning `@oxy.so/bloom/lib`.** That was always required; it is now visible rather than subtle, because the colours moved out of inline style too.
