...
The icon is a component receiving `width`, `height` and `fill`; `children` is not an icon alias. Size and tone inherit from `BloomScope`; `tone` controls the solid active state.
...
| ---------- | ---------------------------------------------------- |
| `icon`     | the glyph                                            |
| `size`     | `xs` · `sm` · `md` · `lg`, or a number                             |
| `checked`   | a persistent on state, for a toggle                  |
