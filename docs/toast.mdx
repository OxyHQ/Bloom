...
`border`, `text` and `textSecondary` roles — whatever its `type`. The variant
shows up only in the leading icon, which uses the theme's first-class status
tokens: `success`, `error`, `warning`, `info`. A `loading` or variant-less toast
gets `textSecondary`.

That is deliberate, and matches sonner. `toast.success('Saved')` should read as a
normal toast with a green check, not as a full brand-coloured card. Opt into the
tinted treatment with `richColors` (below) — never by styling per variant.
...
| `richColors?` | `boolean` | Opt in to a tinted SURFACE. Off by default, where the variant only colours the icon. `success` / `error` get a tinted surface, border and text; `warning` / `info` lift their border and title only, because Bloom has no `warningSubtle` / `infoSubtle` pair. |
