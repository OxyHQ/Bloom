...
<Callout variant="info" title="Historical package names">
This guide records the original 2026 Bloom 1.0 migration. References to the
former `@oxyhq/*` scope and its then-current versions are preserved as historical
evidence; current applications install Bloom and the Oxy SDK from `@oxy.so/*`.
</Callout>
...
`@oxy.so/bloom/design-tokens` plus the `./design-tokens/tokens.json` subpath, and
...
| --- | --- |
| `src/ui/components/FollowTargetButton.tsx` | `@oxy.so/bloom/menu` |
| `src/ui/components/authChooser/TroubleDisclosure.tsx` | `@oxy.so/bloom/collapsible` |
...
mismatch for a package manager to report. `@oxy.so/bloom` declares no `@oxyhq/*`
...
node -e 'for (const p of ["@oxy.so/bloom","@oxyhq/services","@oxyhq/core"]) console.log(p, require(p + "/package.json").version)'
```
...
metadata for `@oxy.so/bloom@1.0.0`, `@oxyhq/services@29.0.0` and `30.0.1`, and
...
Not from commit subjects. The published `@oxy.so/bloom@0.87.1` and `0.89.0`
...
| --- | --- | --- |
| `@oxy.so/bloom/menu` | `@oxy.so/bloom/dropdown-menu` | The family was ported to shadcn/react-native-reusables' public API. Every `Menu*` export is `DropdownMenu*`. |
| `@oxy.so/bloom/collapsible` | `@oxy.so/bloom/accordion` | `Collapsible` called `UIManager.setLayoutAnimationEnabledExperimental(true)` at module scope and reached the root barrel through `export *`, so that side effect ran for every barrel consumer on Android old-arch whether or not the component was used. |
| `@oxy.so/bloom/grouped-buttons` | `@oxy.so/bloom/settings-list` + `@oxy.so/bloom/item` | The only one of the three row renderers with no `accessibilityRole`, no `accessibilityLabel` and no `aria-disabled`, built on `TouchableOpacity`/`activeOpacity` instead of `useInteractionState`. |
| `@oxy.so/bloom/pressable-with-hover` | none — inline it | One internal user. It now lives inside `zoomable-media-gallery` as `NavArrow`, the package's only hover-styled control. |

### Subpaths added

`@oxy.so/bloom/dropdown-menu`, `@oxy.so/bloom/menubar`, `@oxy.so/bloom/radio`,
`@oxy.so/bloom/aspect-ratio`, and the `@oxy.so/bloom/design-tokens/tokens.json`
...
Fifteen flat exports of `@oxy.so/bloom/theme` are now members of the `ColorEngine`
namespace, still published from the same subpath:

```ts
// before
import { argbFromHex, generateRoleColors, quantizeImage } from '@oxy.so/bloom/theme';

// after
import { ColorEngine } from '@oxy.so/bloom/theme';
...
```ts
-import { getPresetVars } from '@oxy.so/bloom/theme';
+import { getPresetVars } from '@oxy.so/bloom/design-tokens';
```

`getPresetVars` and `ExplicitAccents` are also on `@oxy.so/bloom/preset-vars`.

### `alert()` and `confirm()` moved to the surface stack

`alert()` lived on `./dialog`, `confirm()` on `./alert-dialog`. Both are now on
`@oxy.so/bloom/surfaces`, along with `AlertButton` and `AlertButtonStyle`.

```ts
-import { alert } from '@oxy.so/bloom/dialog';
...
| `Chip` | `variant="soft"` → `variant="subtle"`, one spelling matching the token family. `color="info"` now exists (it was on `Badge` only). The `onClose` affordance draws `RiCloseLine`, not the `×` glyph. |
...
A renamed subpath breaks `jest.mock('@oxy.so/bloom/<sub>')` silently — the string
is not an import, so no type checker and no import census sees it.

Measured across the Oxy fleet: **131 `jest.mock('@oxy.so/bloom/…')` call sites in
...
```bash
git grep -nE "@oxy.so/bloom/(menu|collapsible|grouped-buttons|pressable-with-hover)['\"]"
```
</Callout>
...
| Symbol | Sites | Fix |
| --- | --- | --- |
| `getPresetVars` | 10 | import from `@oxy.so/bloom/design-tokens` |
| `confirm` (from `./alert-dialog`) | 12 | import from `@oxy.so/bloom/surfaces` |
...
| `alert` (from `./dialog`) | 3 | import from `@oxy.so/bloom/surfaces` |
...
`@oxy.so/bloom/icons` is byte-identical between 0.87.1 and 1.0.0 — a census will
