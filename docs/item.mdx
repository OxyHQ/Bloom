...
| `highlighted` | the keyboard's cursor, in a list a field drives                  |
...
`highlighted` is the third: *the keyboard is here* — the row an arrow key has
moved to in a list the field above it drives (a combobox, a composer's
suggestions). It paints the hover wash, because it is the same claim as the
pointer resting on the row, and it announces nothing: the row is neither chosen
nor current.
