...
Bloom ships modal primitives, form controls, layout helpers, typography, theming, and a portal system that works on both native and web. Everything is exported under a sub-path (`@oxy.so/bloom/dialog`, `@oxy.so/bloom/button`, etc.) so tree-shaking stays effective.
