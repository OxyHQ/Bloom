...
| prop                 | decides                                       |
| -------------------- | --------------------------------------------- |
| `checked`              | on or off — the component is fully controlled  |
| `onCheckedChange`      | receives the **opposite** of the current value |
| `disabled`           | dims it and takes it out of the tab order      |
| `size`               | `xs` (30×18), `sm` (36×22), `md` (44×26), `lg` (52×30)              |
...
**Inside a `Field` the field's label is that name**, so the prop is not needed
there:
...
The switch takes the field's label as its accessible name, its id (so a click on
the label focuses it on web) and the field's `disabled` — which it cannot opt out
of. The dev-only warning reads the RESOLVED name, so a switch named by its field
does not warn. See [Field](/docs/field).
