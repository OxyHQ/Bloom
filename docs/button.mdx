...
The third row is the interesting one: a backdrop rendered inside a `Modal` — which is what every portaled Bloom overlay is — **can** blur the app root across the window boundary. That is the one place an Android blur is available, and it waits on a decision about who mounts the target.
