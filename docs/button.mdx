...
**One measured cost, stated because it is real.** The 15% bleed moves the pane toward the surface under it, which in light mode means lighter — the one direction a white label cannot afford. Of `primary`'s 180 preset × mode × surface combinations, **30 fall below WCAG AA**, in the band 4.17–4.49, all in light mode, on `blue`, `faircoin`, `green`, `mint`, `orange`, `pumpkin`, `rose`, `sky` and `yellow`. As a solid fill all 180 pass. `destructive` is unaffected (worst 5.32). The alpha at which the failures reach zero is 0.89.

That count was 45 until the surface ramp opened up (`SURFACE_RAMP` in `theme/color-policy.ts`): three of the five surfaces a button sits on are now darker, so a white label on the pane has further to fall before it fails. The nine presets and the 4.17 floor did not move — the worst row sits on `--card`, the one light surface the ramp left alone.
...
### On Android the blur is not applied today

Measured on an Android emulator (API 37): the pane paints its tint and the backdrop behind it is **not blurred**. `expo-blur`'s `blurMethod` defaults to `'none'` on Android, and its `dimezisBlurView` method additionally needs a `blurTarget` ref pointing at the view to blur — something a surface that simply fills its caller cannot supply on its own. Without it `expo-blur` logs one warning and falls back to no blur.

What this changes in practice is small but worth knowing:
...
| --- | --- | --- | --- | --- | --- |
| **0.85** (shipped) | 38.0 | 18 / 4.7 | 1 | 30 / 180 | 0 / 180 |
| 0.75 | 63.7 | 31 / 7.8 | 1 | 80 / 180 | 36 / 180 |
| 0.65 | 88.7 | 43 / 10.9 | 1 | 85 / 180 | 180 / 180 |
| 0.55 | 114.7 | 55 / 14.2 | 1 | 90 / 180 | 180 / 180 |
| 0.45 | 139.3 | 67 / 17.4 | 0 | 102 / 180 | 180 / 180 |
