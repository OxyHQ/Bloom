...
Renders a keyboard shortcut hint on a fully rounded neutral pill, so a
shortcut in running text reads as a key rather than as a stray capital
letter.
...
Painted from the theme's neutral ramp (`button/shared.ts`):

|            | `sm`                 | `md` (default)                 |
| ---------- | -------------------- | ------------------------------ |
| text       | `caption-2-semibold` | `caption-1-semibold` (12/16)   |
| padding    | 4 × 1                | 4 × 2                          |
| height     | 17                   | 20                             |

Fill `neutral-300` (dark `neutral-700`), label `neutral-500` (dark
`neutral-400`), sans family with tracking reset to 0, full-pill radius, no
border. There is no minimum width: a one-glyph key is narrower than it is tall.
