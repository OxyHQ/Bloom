...
`PopoverContent` takes `side` (`'top' | 'bottom'`, default `'bottom'`), `align` (`'start' | 'center' | 'end'`, default `'center'`), `sideOffset`, `alignOffset`, `minWidth` and `maxWidth` (default 360). The surface flips to the other side when the named one does not fit and clamps into the viewport when neither does.
