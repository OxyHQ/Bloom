...
Deliberate, and worth knowing before you wonder where the inset went. A popover's body is as often a list of `Item` rows (an option picker, the dialog header's overflow) as it is prose, and rows have to reach the panel edge to show their full-width highlight. So the panel keeps only the 4px vertical rhythm every anchored Bloom surface has, and a prose popover pads its own body — as the example above does.
...
Rows have to reach the panel's edge to draw a full-width highlight, which is why the dialog header's overflow menu does this.
