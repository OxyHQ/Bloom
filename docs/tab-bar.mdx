...
|------|---------|-------------|
| `expo-glass-effect` | `tab-bar/surface.native` | iOS/Android builds. Provides the iOS 26 liquid-glass capsule; the package's own `isLiquidGlassAvailable()` gate makes every other platform take the solid fallback. |
| `expo-symbols` | `tab-bar/glyph.native` | iOS/Android builds, only if you set `item.sfSymbol`. |
...
`expo-glass-effect` and `expo-symbols` are imported only from `.native` files, and the router packages only from the `expo-router` subpath. A web bundle of `@oxyhq/bloom/tab-bar` reaches none of them, so a web-only app can skip installing them entirely.
</Callout>
