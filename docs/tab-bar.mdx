...
It works with or without a router. `@oxy.so/bloom/tab-bar` has no router dependency at all; the expo-router bindings live in a separate `@oxy.so/bloom/tab-bar/expo-router` subpath.
...
`expo-glass-effect` and `expo-symbols` are imported only from `.native` files, and the router packages only from the `expo-router` subpath. A web bundle of `@oxy.so/bloom/tab-bar` reaches none of them, so nothing about them ends up in a web app's output.
...
import { TabBar, TabBarButton, TabBarMinimizeProvider } from '@oxy.so/bloom/tab-bar';

const TABS = [
  { name: 'home', label: 'Home', icon: <Icons.RiHomeLine /> },
  { name: 'search', label: 'Search', icon: <Icons.RiSearchLine /> },
  { name: 'you', label: 'You', icon: <Icons.RiAccountCircleLine /> },
];
...
} from '@oxy.so/bloom/tab-bar/expo-router';
import { Tabs, TabList, TabSlot, TabTrigger } from 'expo-router/ui';
import { router } from 'expo-router';

const TABS = [
  { name: 'home', label: 'Home', href: '/', icon: <Icons.RiHomeLine /> },
  { name: 'search', label: 'Search', href: '/search', icon: <Icons.RiSearchLine /> },
  { name: 'you', label: 'You', href: '/you', icon: <Icons.RiAccountCircleLine /> },
];
...
Screens opt in with `useMinimizeOnScroll()`, which returns one Reanimated scroll handler for `Animated.ScrollView`, `Animated.FlatList`, or an animated `FlashList`. Scrolling down minimizes the bar to icons only; scrolling up, or being near the top, expands it. The provider owns the shared progress, but cannot infer which descendant is the active scroller; the screen supplies that one explicit binding.
...
If the screen already has a Reanimated scroll handler, pass it in. Bloom uses
Reanimated's event-handler composition, so the consumer's `onScroll`,
`onBeginDrag`, `onEndDrag`, and momentum callbacks remain subscribed and run on
the UI thread:
...
export default function Feed() {
  const feedHandler = useAnimatedScrollHandler({
    onScroll: (event) => {
  ...
  const onScroll = useMinimizeOnScroll(feedHandler);

  return <AnimatedFlatList onScroll={onScroll} scrollEventThrottle={16} />;
}
```

Do not merge these with a JavaScript fan-out callback. That moves per-frame work
off the UI thread and cannot preserve Reanimated's drag and momentum event
subscriptions. Compose processed Reanimated handlers through the hook instead.

The provider deliberately keeps progress when a tab navigator keeps screens
mounted. Bloom does not import a router and cannot know when a route becomes
focused. Expand from the app's navigation focus callback when that is the desired
arrival state:
...
const TABS = [
  {
    name: 'home',
    label: 'Home',
    icon: <Icons.RiHomeLine />,
    sfSymbol: 'house.fill',
  },
  {
    name: 'you',
    label: 'You',
    icon: <Icons.RiAccountCircleLine />,
...
For a complete screen, prefer `AppShell` or `Screen` with `BottomBar`: navigation and the primary action share one row, blur and motion signal. The standalone `TabBar` below remains useful for custom compositions. `embedded` removes its positioning and safe-area ownership; `minimizeProgress` can share a screen motion value. Width is measured from the actual parent on layout.
...
By default the pill grows with its tab count: each expanded tab gets at most a comfortable 88pt, and the whole bar remains centred until enough tabs need the available viewport width. Two or three actions therefore stay compact while larger sets grow naturally. `maxWidth` can impose a tighter ceiling:
...
| `onIndexChange?` | `(index: number) => void` | — | Called once per activation with the chosen index: on tap, on scrub release, or on keyboard / assistive-technology activation. On web and Android the tapped button's own press stands down, because the tap gesture has already reported it. |
...
| `maxWidth?` | `number` | adaptive | Adds a ceiling to the tab-count-based width, with item geometry and hit-testing following. It never stretches a compact bar. |
...
|--------|------|-------------|
| `TabBar`, `TabBarButton` | `@oxy.so/bloom/tab-bar` | The bar and its trigger. |
| `TabBarMinimizeProvider` | `@oxy.so/bloom/tab-bar` | Shares one minimize progress across the tab tree. |
| `useMinimizeOnScroll` | `@oxy.so/bloom/tab-bar` | UI-thread scroll handler for animated scroll views and lists; optionally composes an existing Reanimated handler. |
| `useExpandTabBar` | `@oxy.so/bloom/tab-bar` | Stable expand callback for an app's navigation-focus lifecycle. |
| `useTabBarMinimized` | `@oxy.so/bloom/tab-bar` | The raw `SharedValue<number>`, 0…1. |
| `useTabBarFootprint` | `@oxy.so/bloom/tab-bar` | Space the bar occupies at the bottom, safe-area included, no clearance added. |
| `useMinimizeState`, `setMinimized`, `MINIMIZE_SPRING` | `@oxy.so/bloom/tab-bar` | Lower-level control of the same progress. |
| `RouterTabBar`, `RouterTabBarButton`, `renderFadingTabScreen` | `@oxy.so/bloom/tab-bar/expo-router` | expo-router bindings. |
| `ProgressiveBlur` | `@oxy.so/bloom/progressive-blur` | The gradient blur, standalone. |
...
`TabBar` is app-level navigation chrome: floating, minimize-on-scroll, one per screen tree. For switching between panels *inside* a screen, reach for `Tabs` from `@oxy.so/bloom/tabs` instead — an inline, underlined control with no gestures and no surface of its own.
</Callout>
