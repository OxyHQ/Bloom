...
The bar floats over your content (`position: absolute`), so nothing reserves room for it. `useTabBarReservedSpace()` returns how much a screen must leave at the bottom:

```tsx
import { useMinimizeOnScroll, useTabBarReservedSpace } from '@oxyhq/bloom/tab-bar';
...
export default function Feed() {
  ...
  return (
    <Animated.ScrollView
      ...
      contentContainerStyle={{ paddingBottom: reserved }}
    />
  );
}
```
...
The returned number is measured from the true bottom of the window, because the bar folds the bottom safe-area inset into its own gap. `paddingBottom: reserved + insets.bottom` — the reflex for anything anchored to the bottom edge — counts the home indicator twice and reserves 110pt for a 76pt bar, leaving a visible band of dead space under every list. Use the value as-is; there is nothing to add to it.
</Callout>

It is the bar's **expanded** height, deliberately: the bar minimizes on the way down and re-expands on the way up, so reserving the minimized height would hide the end of a list the moment the user scrolled back to it. Anything else anchored above the bar — a FAB, a docked composer — offsets from the same number (`bottom: reserved + 12`).
...
| `useTabBarReservedSpace` | `@oxyhq/bloom/tab-bar` | Bottom space to leave for the bar, safe-area included. |
