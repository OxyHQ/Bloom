---
title: RadioIndicator
description: The radio dot on its own — presentational, for rows that are already controls.
order: 69
---

# RadioIndicator

The ring-and-dot glyph, with no label, no press target and no group.

```tsx
import { RadioIndicator } from '@oxy.so/bloom/radio-indicator';

<Item
  title={plan.name}
  subtitle={plan.price}
  leading={<RadioIndicator selected={plan.id === chosen} />}
  role="radio"
  onPress={() => setChosen(plan.id)}
/>
```

## Which prop decides what

| prop            | decides                                        |
| --------------- | ----------------------------------------------- |
| `selected`      | filled or hollow                                |
| `size`          | the diameter (16); the dot and hairline scale   |
| `selectedColor` | the accent the selected gradient is built from  |
| `selectedForeground` | foreground paired with a supplied selected fill |
| `borderColor`   | the ring when not                               |

`pressed` is still accepted and ignored: there is no pressed paint.

## What it looks like

The `RadioDot`, in Bloom's colours. At 16px (the `md` size) the inner dot is 6 and the border 1; at 14 (`sm`) 5 and 0.875 — both scale with `size`.

- **Unselected** — the card surface, a neutral-300 border and the `xs` shadow (dark: neutral-800, neutral-700).
- **Selected** — an accent-500 → accent-600 gradient with an inset accent edge and a white top highlight, holding the light inner dot.
- Selecting cross-fades the two surfaces over 200ms (a gradient cannot transition directly) while the dot scales in.

## Use it only where the row is already the control

`RadioIndicator` is **presentational**. It has no press handler and contributes
no role or state, so a group built out of these announces as decoration unless
the surrounding row supplies both — `role="radio"` and the selected state.

If you are building a plain list of options, use
[`Radio` / `RadioGroup`](/docs/bloom/main/radio) instead: they render the label,
make the whole row the target, and name the group. Reaching for the indicator
where you meant the control is the usual way a "radio group" ends up
unreachable by keyboard and silent to a screen reader.

The legitimate cases are rows that are already something else: a card selector,
a table row, a settings row with its own affordances.

## The dot colour is derived, not white

When `selectedColor` is omitted, the dot uses the preset's readable
foreground — dark on a yellow primary, light on a blue one. A hardcoded white
dot disappears on a light accent, which is the same class of failure as
appending alpha to a token: correct markup, invisible control.

Shared radio controls pass `selectedForeground` with their semantic fill. A caller-supplied `selectedColor` without that foreground falls back to white, because Bloom cannot
know the right contrast for an arbitrary colour. If you pass one, check it.

## Selected drops the border

A filled circle with a ring around it reads as two states at once. The bordered
surface and the selected gradient are separate layers that cross-fade, so the
neutral border is never drawn over the fill: once `selected` settles, the only
edge is the gradient's own inset accent hairline, and the fill carries the state.
