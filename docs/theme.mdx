...
The app-wide provider also applies the resolved background and `color-scheme`
to `html` and `body`, covering the document canvas exposed at viewport edges
and during overscroll. It places one managed `theme-color` meta before existing
entries so browser chrome follows the active app palette, including manual
light/dark choices and dynamic seeds. Existing media-qualified entries are
preserved as startup fallbacks. Color and seed scopes do not change these
document-wide settings.

These settings take effect when the provider mounts. Prerendered apps should
still supply their startup background and theme metadata in the HTML/CSS.
