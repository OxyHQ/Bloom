...
<TextField>
  <TextFieldInput nativeID="workspace-name" label="Workspace name" value={name} onValueChange={setName} />
</TextField>
```
...
| `size`     | `xs` body-2-medium · `sm` body-medium (default) · `md` headline-medium |

## What it looks like

The field label, identical to `TextFieldLabel`: Inter `body-medium` in the primary text colour, 4px above the control, with the required asterisk 2px after the text in the error colour. `disabled` drops the text to the tertiary colour.
