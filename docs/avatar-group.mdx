...
who these people are, and an app that wants a fuller preview wraps an avatar in a
[`HoverCard`](/docs/bloom/main/hover-card) with `UserHoverCard` inside.

The card floats in the same panel as [`HoverCardContent`](/docs/bloom/main/hover-card),
so placement (flip and clamp), motion, Escape and outside-press dismissal and the
pointer bridge are the hover card's — including why that bridge must never be a
`Pressable`. What this family adds is ONE card for the whole pile: it re-anchors
as the cursor moves between faces instead of closing and reopening per avatar,
and it opens with no delay, because a pointer on a row of people is already
asking who they are.
