...
The border is **0.5px**, `BORDER_WIDTH.hairline`, the same width
[`BenefitList`](/docs/bloom/main/benefit-list) draws. That is worth stating
because it renders 0.5px on web, where `StyleSheet.hairlineWidth` — the older
spelling — is hardcoded to 1: the two surfaces both called themselves hairline
and did not match. Bloom's scale defines the hairline as 0.5, and this card
follows the scale.
