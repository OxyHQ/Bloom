...
```tsx
import { USER_HOVER_CARD_CONTENT_WIDTH } from '@oxyhq/bloom/user-hover-card';
```

**256px**, and it is exported so you can size against it rather than discover it.
The card is `USER_HOVER_CARD_WIDTH` (288) with `USER_HOVER_CARD_INSET` (16) on
...
`cellSize={11} gap={3}` is 249px. 256 clears it with room, sits on the 8px grid
the spacing scale follows, and is 16× the inset.
