...
- `dynamicBackdrop?: boolean` — defaults to `true`. The backdrop dim and native blur intensity follow the drag continuously: fully visible at rest and fully gone once the sheet leaves the viewport. Set it to `false` only for a deliberately fixed backdrop. The base `backdropOpacity` still controls the resting dim.
