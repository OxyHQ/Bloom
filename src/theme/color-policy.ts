 ...
 */
import { createChartColors } from './chart-colors';
...
/**
 * The tonal surface ramp, owned here as TONES rather than taken from M3's
 * container roles.
 *
 ...
 * row, a skeleton) read as nothing at all. In that historical low-chroma
 * measurement across all 64 presets, 4 tones was ~2.4 ΔE00. The new chromatic
 * palette preserves that tone spacing; its quantized neighbour gaps are tested
 * directly rather than inheriting the old ΔE00 claim.
 *
 * Chromatic light cards stop at tone 98 so they retain the seed hue. The
 * rest of that ladder moves down two tones with them, preserving four-tone
 * separation. Monochrome keeps its original white-card ladder. Dark keeps
 * the original page4 / surface10 / popover14 / muted18 / card22 spacing.
 *
...
/** Seed-linked chroma for the whole shell, not merely its accent controls. */
...
export function buildPolicyTokens(
  ...
  // Surfaces, text and boundaries share the identity hue and tonal authority.
  const neutral = TonalPalette.fromHueAndChroma(seed.hue, monochrome ? 0 : NEUTRAL_CHROMA);
  const originalRamp = isDark ? SURFACE_RAMP.dark : SURFACE_RAMP.light;
  const lightShift = !isDark && !monochrome ? -2 : 0;
  const rampTones = {
    surface: originalRamp.surface + lightShift,
    popover: originalRamp.popover + lightShift,
    muted: originalRamp.muted + lightShift,
  };
  const foreground = rgb(neutral.tone(isDark ? 94 : 10));
  const mutedForeground = rgb(neutral.tone(isDark ? 80 : 30));
  const boundary = rgb(neutral.tone(isDark ? 35 : 72));
  ...
  const tokens: PolicyTokens = {
    ...
    '--card-foreground': monochrome ? roles.onSurface : foreground,
    '--popover-foreground': monochrome ? roles.onSurface : foreground,
    '--sidebar-foreground': monochrome ? roles.onSurface : foreground,
    '--muted-foreground': monochrome ? roles.onSurfaceVariant : mutedForeground,
    '--border': monochrome ? roles.outlineVariant : boundary,
    '--input': monochrome ? roles.outlineVariant : boundary,
    '--sidebar-border': monochrome ? roles.outlineVariant : boundary,
    ...
    '--accent-foreground': monochrome ? roles.onSurfaceVariant : mutedForeground,
    '--sidebar-accent': monochrome ? roles.secondaryContainer : rgb(neutral.tone(rampTones.muted)),
    '--sidebar-accent-foreground': monochrome ? roles.onSecondaryContainer : mutedForeground,
  };
  ...
  // CSS and component palettes share all nine categorical fill/active pairs.
  // Existing chart-1…5 fills remain byte-for-byte stable.
  createChartColors(seed.hue, isDark, monochrome).forEach((pair, index) => {
    tokens[`--chart-${index + 1}`] = pair.color;
    tokens[`--chart-${index + 1}-active`] = pair.activeColor;
  });
