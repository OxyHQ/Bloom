/**
 * The renewed `Popover` panel: BoardUI's floating surface, resolved inline.
 *
 ...
 *     `style` after it, with BoardUI's 8px offset — asserted on a mocked
 *     `FloatingPanel`, since the real one needs a measured anchor.
 *  3. The panel parts render the ramp step and palette colour BoardUI uses, and
...
describe('resolvePopoverSurfaceStyle', () => {
  ...
  it.each(['light', 'dark'] as const)('paints BoardUI’s panel from the %s menu palette', (mode) => {
...
describe('panel parts', () => {
  ...
  it('renders BoardUI’s header, title, description, footer and separator', () => {
    const p = palette();
    const screen = render(
      <Themed>
        <PopoverHeader testID="header" leading={<RNText>A</RNText>}>
          <PopoverTitle testID="title">Board team</PopoverTitle>
          <PopoverDescription testID="description">hi@boardui.com</PopoverDescription>
        </PopoverHeader>
