...
export function BottomBarBase({ Navigation, Item, Blur, items, value, onValueChange, action,
  ...
  return <View testID={testID} pointerEvents="box-none" style={[{ height: bottom + EXPANDED_HEIGHT, justifyContent: 'flex-end' }, style]}>
    ...
    <Animated.View onLayout={event => setRowWidth(event.nativeEvent.layout.width)} pointerEvents="box-none" style={[{ justifyContent: items.length ? 'center' : 'flex-end', alignSelf: 'center', width: '100%', maxWidth, paddingLeft: 12, paddingRight: 12, marginBottom: bottom, flexDirection: 'row', alignItems: 'flex-end' }, rowStyle]}>
