...
export function Screen({ header, bottomBar, primaryAction, active: requestedActive = true, navigationScope = 'inherit', documentScroll = false, headerHeight, bottomBarHeight, contentClearance = 16, children, style, testID, ...props }: ScreenProps) {
  ...
  const value = useMemo(() => ({ active, scrollY, collapseProgress, collapseTarget, activeScrollerId,
    topInset: header ? top : 0, bottomInset: hasBottom ? bottom + contentClearance : navigationScope === 'inherit' ? inheritedNavigation?.bottomInset ?? contentClearance : contentClearance,
  }), [active, scrollY, collapseProgress, collapseTarget, activeScrollerId, header, hasBottom, top, bottom, contentClearance, navigationScope, inheritedNavigation?.bottomInset]);
  ...
  return (
    ...
    <ScreenContext.Provider value={value}>
      <View {...props} testID={testID} style={[{ flex: 1, minHeight: 0, position: 'relative', backgroundColor: colors.background }, style]}>
