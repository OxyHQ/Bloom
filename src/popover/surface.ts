/**
 * The popover PANEL — BoardUI's one floating surface, resolved to inline style.
 *
 * BoardUI has no standalone popover component, but every floating panel it ships
 * (`dropdown/menu-styles.ts`'s `MENU_POPOVER_SURFACE`, the dashboard team/user
 * menus, the calendar inbox menu, the ai-chat menus) spells the same class
 * string:
 *
...
/** BoardUI popovers sit `offset={8}` from their trigger. */
...
export function resolvePopoverSurfaceStyle(
  ...
  // BoardUI's `overflow-y-auto` clips a full-bleed child to the 16px corner and
