...
import type { PromptInputProps, PromptInputTextareaProps } from './types';

export interface PromptInputBaseProps extends PromptInputProps {
  /**
   * The textarea to render in simple mode (no `children`). Supplied by the
   * platform binding — `PromptInput.tsx` passes the React Native input,
   * `PromptInput.web.tsx` the web fork, which adds paste-to-attach and the
   * `onImagePaste` listener. Taking it as a prop is what keeps ONE
   * implementation of the surrounding chrome: forking this file would
   * duplicate 250 lines to change one element.
   */
  Textarea: React.ComponentType<PromptInputTextareaProps>;
}

export function PromptInputBase({
  Textarea,
  isLoading = false,
  maxHeight = 240,
  value,
...
PromptInputBase.displayName = 'PromptInputBase';
